...
sim.add(m=1.0, y=0.0, vx=0.001, vy=-0.1,r=0.005) #Star 2
...
while sim.t < max_time:
    ...
    if genvel(sp[2]) >= math.sqrt(2 * sim.G * ((sp[0].m)/gendist1(sp) + (sp[1].m)/gendist2(sp))):
        ...
        break
    if sim.t == max_time:
        print("System was stable for longer than 1000 years.")
        break
    plt.plot(sp[0].x, sp[0].y, color = 'red', marker='o', markersize = 1)
    plt.plot(sp[1].x, sp[1].y, color = 'blue', marker='o', markersize = 1)
    plt.plot(sp[2].x, sp[2].y, color = 'green', marker='o', markersize = 1)
plt.xlabel('x (AU)')
plt.ylabel('y (AU)')
